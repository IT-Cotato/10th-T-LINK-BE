...
public class LectureFileBoxFacade {
			...
			.map(file -> {
				try {
					return LectureFileResponse.from(file.getId(), file.getOriginalName(),
						s3FileHandler.downloadFile(FolderPath.generate(FolderPath.LECTURE, file.getS3Key())).getURL().toString()
					);
				} catch (IOException e) {
	...
	public void removeLectureFileBox(final Long lectureFileBoxId) {
		...
		lectureFiles.forEach(lectureFile -> s3FileHandler.deleteFile(FolderPath.generate(FolderPath.LECTURE, lectureFile.getS3Key())));
	...
	private void saveLectureFiles(final List<MultipartFile> lectureFiles, final LectureFileBox lectureFileBox) throws
		...
		for (int i=0; i<size; i++) {
			...
			s3FileHandler.uploadFile(lectureFiles.get(i), FolderPath.generate(FolderPath.LECTURE, filePaths.get(i)));	// 파일 업로드
